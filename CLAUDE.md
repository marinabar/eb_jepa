...
A **hierarchical encoder** with inductive biases built on the hallmark pathways is implemented in `eb_jepa.singlecell.sub14.hierarchical` (`HierarchicalSubliminal14`). It adds a second, abstract attention level over the 50 hallmark pathways, interleaved with the gene-level sigmoid-attention blocks via three masked steps: **pool** (genes → pathways: one learned query token per pathway aggregates, by masked cross-attention, the hidden states of its member genes), **mix** (pathways ↔ pathways: a gene-level-identical pre-norm sigmoid block), and **scatter** (pathways → genes + `[CELL]`: every gene reads back from the pathways it belongs to, the `[CELL]` token from all present pathways). Genes sharing a hallmark pathway thus communicate through the abstract hierarchy. The LeJEPA loss and the `[CELL]` readout are unchanged, and every gene-level module keeps sub14's names/shapes so a trained sub14 checkpoint warm-starts the model key-for-key (only the `encoder.hier_blocks.*` pathway level is freshly initialised). A pathway block runs after each gene layer in `hier_positions` (default: after every gene layer but the last). The PC-local membership comes from `pathway_membership.pt` (built by `scripts/build_pathway_membership.py`, sliced to the protein-coding vocabulary by `load_pathway_membership`); driven by `model.hierarchical: true` in the config (`cfgs/sub14_small_hier.yaml`).
