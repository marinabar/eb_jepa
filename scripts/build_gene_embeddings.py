...
"Gene-embedding cache (ESMC + Evo2)"):
  ...
  * ESMC pooled protein embeddings  -> ``[N_coding, 1280]``  (coding genes only)
...
62,710 genes, Ensembl release 109 / GRCh38):
  ...
  2. ESMC ``esmc-600m`` (1280-dim/residue) over the protein, mean-pooled -> 1 vector
     per coding gene.
  3. Evo2 (default ``arcinstitute/evo2_7b``, ``d_evo2`` READ FROM THE MODEL CONFIG)
...
# ESMC 600M emits a fixed 1280-d per-residue representation. This is a property of
...
class Config:
    ...
    # --- Evo2 (DNA) -------------------------------------------------------------
    evo2_model: str = "arcinstitute/evo2_7b"  # also: evo2_1b_base, evo2_7b_base, ...
    evo2_layer: int = 24  # extraction layer (chosen by sweep)
...
class ESMCEmbedder:
    ...
    def _ensure_model(self):
        if self._model is not None:
            return
        # TODO-verify-on-cluster: confirm the exact ESMC import + forward API against
        # the installed `esm` package version. As of the public release:
        #   from esm.models.esmc import ESMC
        #   from esm.sdk.api import ESMProtein, LogitsConfig
        #   model = ESMC.from_pretrained("esmc_600m").to(device)
        # The embedding call is roughly:
        #   p = model.encode(ESMProtein(sequence=seq))
        #   out = model.logits(p, LogitsConfig(sequence=True, return_embeddings=True))
        #   per_residue = out.embeddings   # [1, L+special, 1280]
        # If the exact names differ, adjust here only.
        from esm.models.esmc import ESMC  # noqa: F401

        logger.info("Loading ESMC model %s ...", self.cfg.esmc_model)
...
class Evo2Embedder:
    ...
    def _ensure_model(self):
        if self._model is not None:
            return
        # TODO-verify-on-cluster: confirm the exact Evo2 import + embedding-extraction
        # API against the installed `evo2` package. As of the public release:
        #   from evo2 import Evo2
        #   model = Evo2(self.cfg.evo2_model.split("/")[-1])   # e.g. "evo2_7b"
        #   _, embeddings = model(input_ids, return_embeddings=True,
        #                         layer_names=[f"blocks.{layer}.mlp.l3"])
        # The layer NAME convention ("blocks.{i}...") must be checked on cluster; we
        # expose `layer` as an int and build the name in `_layer_name`.
    ...
    def _read_hidden_dim(model) -> int:
        ...
        TODO-verify-on-cluster: confirm the actual config attribute name for the
        installed evo2 version (candidates below cover known variants).
        """
        cfg_obj = getattr(model, "config", None) or getattr(model, "model_config", None)
        candidates = ("hidden_size", "d_model", "hidden_dim", "model_dim", "embed_dim")
    ...
    def _layer_name(self, layer: int) -> str:
        # TODO-verify-on-cluster: Evo2 indexes intermediate activations by string
        # layer name. Confirm the exact pattern; this is the documented one.
        return f"blocks.{layer}.mlp.l3"

    def _tokenize(self, seq: str):
        import torch

        # TODO-verify-on-cluster: Evo2 ships a tokenizer (`model.tokenizer`). The
        # public API is roughly `ids = model.tokenizer.tokenize(seq)`; wrap to tensor.
    ...
    def _forward_layer(self, seq: str, layer: int) -> "np.ndarray":
        ...
        with torch.no_grad():
            # TODO-verify-on-cluster: signature of the embedding-returning forward.
...
# =============================================================================
def merge_shards(cfg: Config) -> None:
    """TODO: concatenate per-shard row-stores into a single global cache.

    When run with ``--num_shards N`` across N GPUs, each process writes
    ``out_dir/shard_i/{esmc,evo2}.npy`` + its own index. This pass should:
      1. read all shard indices, re-number ``esmc_row``/``evo2_row`` into a global
         contiguous layout ordered by ``token_id``,
      2. write the merged ``esmc.npy`` / ``evo2.npy`` memmaps + a single
         ``index.parquet`` + ``metadata.json`` at ``out_dir``.
    Left as a hook; the single-GPU path writes the final cache directly.
    """
    raise NotImplementedError(
        "merge_shards: implement when running multi-shard builds."
    )
